...
const assetUrl=v=>!v?'':/^https?:\/\//i.test(String(v))?String(v):`./${String(v).replace(/^\.?\//,'')}`;
...
function normalizeCatalog(raw){
  ...
  PROFESSIONALS=(raw?.professionals||[]).map(p=>({id:p.id,slug:p.slug,name:p.name,image:assetUrl(p.image_path)}));
}
...
const itemLink=(item,professionalId=state.booking.professionalId)=>professionalId?(item?.professionalLinks||[]).find(x=>x.professional_id===professionalId):null;
const itemPrice=item=>{const l=itemLink(item);return Number(l?.price_cents??item?.priceCents??0)/100};
const itemDuration=item=>{const l=itemLink(item);return Number(l?.duration_min??item?.duration??0)};
const selectedPrice=()=>itemPrice(serviceById(state.booking.serviceId))+selectedAddons().reduce((x,a)=>x+itemPrice(a),0);
const selectedDuration=()=>itemDuration(serviceById(state.booking.serviceId))+selectedAddons().reduce((x,a)=>x+itemDuration(a),0);
const eligibleProfessionals=()=>{
  const service=serviceById(state.booking.serviceId);if(!service)return[];
  const required=[service,...selectedAddons()];
  return PROFESSIONALS.filter(p=>required.every(item=>(item.professionalIds||[]).includes(p.id)));
};
...
function bookingServiceLabel(b){
  const base=b.service?.name||serviceById(b.serviceId)?.name||'';
  const adds=(b.rawAddons||[]).map(a=>a.name);
  return adds.length?`${base} + ${adds.join(' + ')}`:base;
}
function bookingDuration(b){
  const base=Number(b.service?.duration_min??serviceById(b.serviceId)?.duration??0);
  return base+(b.rawAddons||[]).reduce((s,a)=>s+Number(a.duration_min||0),0);
}
...
function renderHome(){
  ...
  if(next){
    const pro={name:next.professional?.name||proById(next.professionalId)?.name||''};
    wrap.innerHTML=`<section class="next-card ${isToday(next.date)?'today':''}"><div class="next-head"><div><span class="kicker">${isToday(next.date)?'Seu horário é hoje':'Seu próximo horário'}</span><h3>${esc(bookingServiceLabel(next))}</h3></div><span class="next-badge">${next.time}</span></div><div class="next-details"><div class="detail-box"><small>Data</small><strong>${humanDate(next.date,false)}</strong></div><div class="detail-box"><small>Profissional</small><strong>${esc(pro.name)}</strong></div><div class="detail-box"><small>Duração</small><strong>${bookingDuration(next)} min</strong></div><div class="detail-box"><small>Valor</small><strong>${money(bookingPrice(next))}</strong></div></div><div class="card-actions"><button class="secondary-button" type="button" data-home-reschedule="${next.id}">Reagendar</button><button class="danger-button" type="button" data-home-cancel="${next.id}">Cancelar</button></div></section>`;
...
function startReschedule(id){const b=allBookings().find(x=>x.id===id);if(!b)return;if(!serviceById(b.serviceId)){toast('Esse serviço não está mais disponível para reagendamento.');return}state.bookingMode='reschedule';state.editingBookingId=id;state.booking={serviceId:b.serviceId,addons:[...b.addons],professionalId:b.professionalId,date:b.date,time:b.time,customer:currentProfile()};state.bookingStep=1;showView('booking');renderBooking()}
function startRepeat(id){const b=allBookings().find(x=>x.id===id);if(!b)return;if(!serviceById(b.serviceId)){toast('Esse serviço não está mais disponível para um novo agendamento.');return}resetBooking();state.booking.serviceId=b.serviceId;state.booking.addons=[...b.addons];state.booking.professionalId=b.professionalId;showView('booking');renderBooking()}
...
function bookingServices(){
  ...
  const addonHTML=service&&suggestions.length?`<section class="addon-section"><div class="addon-heading"><span class="kicker">Complete seu atendimento</span><h3>Que tal adicionar?</h3><p>Opcional. Você pode continuar sem nenhum adicional.</p></div><div class="addon-list">${suggestions.map(a=>{const selected=state.booking.addons.includes(a.id);return `<button class="addon-card ${selected?'selected':''}" type="button" data-addon="${a.id}"><img src="${a.image}" alt="${esc(a.name)}"><span class="addon-info"><strong>${esc(a.name)}</strong>${a.description?`<span class="service-description">${esc(a.description)}</span>`:''}<small>${a.durationLabel}</small><span class="addon-price"><b>+ ${money(a.price)}</b></span></span><span class="addon-action">${selected?'✓':'+'}</span></button>`}).join('')}</div></section>`:'';
  stage.innerHTML=`<div class="booking-list">${SERVICES.map(s=>`<button class="choice-service ${state.booking.serviceId===s.id?'selected':''}" type="button" data-service="${s.id}"><img src="${s.image}" alt=""><span><h3>${esc(s.name)}</h3>${s.description?`<span class="service-description">${esc(s.description)}</span>`:''}${s.kind==='combo'&&s.components?.length?`<span class="combo-components">Inclui: ${s.components.map(c=>esc(c.name)).join(' + ')}</span>`:''}<p><strong>${money(s.price)}</strong> · ${s.durationLabel}</p></span><span class="radio-mark"></span></button>`).join('')}</div>${addonHTML}${state.booking.serviceId?`<div class="booking-total-mini"><span>Total selecionado</span><strong>${money(selectedPrice())}</strong></div><button class="gold-button continue-btn" id="serviceNext" type="button">Continuar →</button>`:''}`;
  $$('[data-service]',stage).forEach(b=>b.onclick=()=>{if(state.booking.serviceId!==b.dataset.service)state.booking.addons=[];state.booking.serviceId=b.dataset.service;state.booking.professionalId=null;state.booking.date=null;state.booking.time=null;bookingServices()});
  $$('[data-addon]',stage).forEach(b=>b.onclick=()=>{const set=new Set(state.booking.addons);set.has(b.dataset.addon)?set.delete(b.dataset.addon):set.add(b.dataset.addon);state.booking.addons=[...set];if(state.booking.professionalId&&!eligibleProfessionals().some(p=>p.id===state.booking.professionalId))state.booking.professionalId=null;state.booking.date=null;state.booking.time=null;bookingServices()});
  $('#serviceNext')?.addEventListener('click',stepNext);
}
function bookingProfessionals(){const stage=$('#bookingStage'),pros=eligibleProfessionals();if(state.booking.professionalId&&!pros.some(p=>p.id===state.booking.professionalId))state.booking.professionalId=null;stage.innerHTML=pros.length?`<div class="booking-list">${pros.map(p=>{const service=serviceById(state.booking.serviceId),link=itemLink(service,p.id),price=Number(link?.price_cents??service.priceCents)/100,duration=Number(link?.duration_min??service.duration);return `<button class="pro-choice ${state.booking.professionalId===p.id?'selected':''}" type="button" data-pro="${p.id}" data-pro-slug="${p.slug}"><img src="${p.image}" alt="${esc(p.name)}"><span class="pro-choice-copy"><strong>${esc(p.name)}</strong><small>${money(price)} · ${duration} min</small></span><span class="radio-mark"></span></button>`}).join('')}</div>${state.booking.professionalId?'<button class="gold-button continue-btn" id="proNext" type="button">Continuar →</button>':''}`:'<div class="no-times">Nenhum profissional está vinculado a este serviço e aos adicionais selecionados.</div>' ;$$('[data-pro]',stage).forEach(b=>b.onclick=()=>{state.booking.professionalId=b.dataset.pro;state.booking.date=null;state.booking.time=null;bookingProfessionals()});$('#proNext')?.addEventListener('click',stepNext)}
function availableDates(){const days=Number(CATALOG?.unit?.settings?.days_ahead??30),arr=[],today=new Date();for(let i=0;i<=days;i++){const d=new Date(today);d.setHours(12,0,0,0);d.setDate(today.getDate()+i);arr.push(d)}return arr}
function bookingDates(){const stage=$('#bookingStage'),dates=availableDates(),monthFmt=new Intl.DateTimeFormat('pt-BR',{month:'long',year:'numeric'});stage.innerHTML=`<p class="month-label">${monthFmt.format(dates[0])}</p><div class="calendar-strip">${dates.map(d=>{const iso=localDateISO(d),wd=new Intl.DateTimeFormat('pt-BR',{weekday:'short'}).format(d).replace('.',''),mon=new Intl.DateTimeFormat('pt-BR',{month:'short'}).format(d).replace('.','');return `<button class="day-button ${state.booking.date===iso?'selected':''}" type="button" data-date="${iso}"><small>${wd}</small><strong>${pad(d.getDate())}</strong><span>${mon}</span></button>`}).join('')}</div>${state.booking.date?'<button class="gold-button continue-btn" id="dateNext" type="button">Ver horários →</button>':''}`;$$('[data-date]',stage).forEach(b=>b.onclick=()=>{state.booking.date=b.dataset.date;state.booking.time=null;bookingDates()});$('#dateNext')?.addEventListener('click',stepNext)}
...
function bookingReview(){const stage=$('#bookingStage'),s=serviceById(state.booking.serviceId),p=proById(state.booking.professionalId),c=state.booking.customer;stage.innerHTML=`<div class="review-card"><div class="review-hero"><img src="${s.image}" alt=""><div><h3>${esc(s.name)}</h3><p>${money(selectedPrice())}</p></div></div><div class="review-lines"><div class="review-line"><span>Profissional</span><strong>${esc(p.name)}</strong></div><div class="review-line"><span>Data</span><strong>${humanDate(state.booking.date)}</strong></div><div class="review-line"><span>Horário</span><strong>${state.booking.time}</strong></div>${selectedAddons().map(a=>`<div class="review-line"><span>Adicional</span><strong>${esc(a.name)} · + ${money(itemPrice(a))}</strong></div>`).join('')}<div class="review-line"><span>Duração total</span><strong>${selectedDuration()} min</strong></div><div class="review-line"><span>Total</span><strong>${money(selectedPrice())}</strong></div><div class="review-line"><span>Cliente</span><strong>${esc(c.name)}</strong></div></div></div><div class="stage-actions"><button class="secondary-button" id="reviewBack" type="button">Voltar e alterar</button><button class="gold-button" id="confirmBooking" type="button">${state.bookingMode==='reschedule'?'Confirmar novo horário':'Confirmar agendamento'}</button></div>`;$('#reviewBack').onclick=()=>{state.bookingStep=1;renderBooking()};$('#confirmBooking').onclick=commitBooking}
...
function renderAppointments(){
  ...
  root.innerHTML=list.map((b,i)=>{const currentPro=proById(b.professionalId),p={name:b.professional?.name||currentPro?.name||'',image:assetUrl(b.professional?.image_path)||currentPro?.image||''},up=state.appointmentsTab==='upcoming';return `<article class="appointment-card ${up&&i===0?'featured':''}"><div class="appt-top"><div><h3>${esc(bookingServiceLabel(b))}</h3><p>${humanDate(b.date)} • ${b.time} • ${bookingDuration(b)} min</p></div><span class="appt-price">${money(bookingPrice(b))}</span></div><div class="appt-pro"><img src="${p.image}" alt=""><strong>${esc(p.name)}</strong></div><div class="appt-actions">${up?`<button class="secondary-button" type="button" data-reschedule="${b.id}">Reagendar</button><button class="danger-button" type="button" data-cancel="${b.id}">Cancelar</button>`:`<button class="secondary-button" type="button" data-repeat="${b.id}">Agendar novamente</button>`}</div></article>`}).join('');
