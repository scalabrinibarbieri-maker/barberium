...
let commissionTab='rules';
let myCommissionFilter='receivable';
let commissionSettlementCache={professionals:[],settlements:[]};
...
function paymentMethodLabel(v){return({pix:'Pix',cash:'Dinheiro',debit:'Débito',credit:'Crédito',transfer:'Transferência',other:'Outro'})[v]||v}
...
async function loadCommissions(){
  if(!isManager())return loadMyCommissions();
  $('#commissionManagerSubtabs')?.classList.remove('hidden');$('#commissionBarberSubtabs')?.classList.add('hidden');
  if(commissionTab==='settlements')return loadCommissionSettlements();
  if(commissionTab==='plans')return loadPlanCommissionList();
  $('#commissionContent').innerHTML='<div class="loading">Carregando regras de comissão…</div>';
...
function commissionSettlementCard(x){const remaining=Number(x.remaining_cents||0);return `<button class="plan-card commission-settlement-card" data-settlement-id="${x.id}" type="button"><div class="plan-head"><div><span class="plan-tag">${financeStatusLabel(x.status)}</span><h3>${esc(x.professional||'Comissão')}</h3><p>${brDateOnly(x.period_start)} — ${brDateOnly(x.period_end)}</p></div><strong>${moneyCents(x.total_cents)}</strong></div><div class="plan-meta"><span>Base ${moneyCents(x.base_cents)}</span><span>Ajustes ${moneyCents(x.adjustment_cents)}</span><span>Pago ${moneyCents(x.paid_cents)}</span><span>${remaining>0?`Falta ${moneyCents(remaining)}`:'Quitado'}</span></div></button>`}
async function loadCommissionSettlements(){
  $('#commissionContent').innerHTML='<div class="loading">Carregando acertos…</div>';
  try{commissionSettlementCache=await rpc('barberium_staff_commission_settlements',{});const pros=commissionSettlementCache.professionals||[],rows=commissionSettlementCache.settlements||[];$('#commissionContent').innerHTML=`<div class="client-toolbar"><button id="newCommissionSettlement" class="gold-btn" type="button">+ Novo acerto</button></div><div class="finance-summary">${pros.map(p=>`<article class="finance-kpi ${Number(p.due_cents||0)>0?'gold':''}"><small>${esc(p.name)}</small><strong>${moneyCents(p.due_cents)}</strong><span>${moneyCents(p.unsettled_cents)} a fechar • ${moneyCents(p.in_settlement_cents)} em acertos</span></article>`).join('')}</div><h3 class="section-mini-title">Histórico de acertos</h3>${rows.length?rows.map(commissionSettlementCard).join(''):'<div class="empty">Nenhum acerto criado ainda.</div>'}`;$('#newCommissionSettlement').onclick=openNewCommissionSettlement;$$('[data-settlement-id]').forEach(b=>b.onclick=()=>openCommissionSettlementDetail(b.dataset.settlementId))}catch(e){$('#commissionContent').innerHTML=`<div class="empty">${esc(friendlyError(e))}</div>`}
}
async function openNewCommissionSettlement(){
  try{if(!(commissionSettlementCache.professionals||[]).length)commissionSettlementCache=await rpc('barberium_staff_commission_settlements',{});const [start,end]=financeMonthRange();const pros=commissionSettlementCache.professionals||[];openModal('COMISSÕES','Novo acerto',`<form id="commissionSettlementForm" class="form-grid"><div class="field"><label>Profissional</label><select id="settlementProfessional">${pros.map(p=>`<option value="${p.id}">${esc(p.name)} • ${moneyCents(p.unsettled_cents)} a fechar</option>`).join('')}</select></div><div class="field"><label>Início do período</label><input id="settlementStart" type="date" value="${start}"></div><div class="field"><label>Fim do período</label><input id="settlementEnd" type="date" value="${end}"></div><div class="field wide"><label>Observação do fechamento</label><textarea id="settlementNote" placeholder="Opcional"></textarea></div><button id="previewSettlement" class="soft-btn" type="button">Ver comissões do período</button><div id="settlementPreview" class="wide"><div class="empty">Escolha o período e veja os lançamentos antes de fechar.</div></div></form>`);$('#previewSettlement').onclick=previewCommissionSettlement}catch(e){toast(friendlyError(e))}
}
async function previewCommissionSettlement(){const pro=$('#settlementProfessional').value,start=$('#settlementStart').value,end=$('#settlementEnd').value,wrap=$('#settlementPreview');wrap.innerHTML='<div class="loading">Buscando lançamentos…</div>';try{const d=await rpc('barberium_staff_commission_preview',{p_professional_id:pro,p_start:start,p_end:end});wrap.innerHTML=`<div class="detail-grid"><div class="detail-box"><small>Lançamentos</small><strong>${d.count}</strong></div><div class="detail-box"><small>Total do fechamento</small><strong>${moneyCents(d.total_cents)}</strong></div></div><h3 class="section-mini-title">Comissões incluídas</h3>${(d.entries||[]).map(e=>`<article class="service-stat"><div><h3>${esc(e.appointment?.service||e.description||'Comissão')}</h3><p>${e.appointment?`${esc(e.appointment.customer)} • `:''}${dateTimeBR(e.created_at)}${e.rate_percent!=null?` • ${Number(e.rate_percent).toLocaleString('pt-BR')}%`:''}</p></div><strong>${moneyCents(e.amount_cents)}</strong></article>`).join('')||'<div class="empty">Nenhuma comissão aberta neste período.</div>'}${d.count?'<button id="confirmSettlement" class="gold-btn finance-wide" type="button">Fechar período e criar acerto</button>':''}`;$('#confirmSettlement')?.addEventListener('click',createCommissionSettlement)}catch(e){wrap.innerHTML=`<div class="empty">${esc(friendlyError(e))}</div>`}}
async function createCommissionSettlement(){const btn=$('#confirmSettlement');btn.disabled=true;try{const r=await rpc('barberium_staff_create_commission_settlement',{p_professional_id:$('#settlementProfessional').value,p_period_start:$('#settlementStart').value,p_period_end:$('#settlementEnd').value,p_note:$('#settlementNote').value.trim()||null});toast(`Acerto criado: ${moneyCents(r.total_cents)}.`);closeModal();await loadCommissionSettlements()}catch(e){toast(friendlyError(e));btn.disabled=false}}
async function openCommissionSettlementDetail(id){openModal('ACERTO DE COMISSÃO','Carregando…','<div class="loading">Buscando detalhes…</div>');try{const d=await rpc('barberium_staff_commission_settlement_detail',{p_settlement_id:id});const x=d.settlement;$('#modalTitle').textContent=x.professional;const canManage=isManager()&&x.status!=='paid';$('#modalBody').innerHTML=`<div class="detail-grid"><div class="detail-box"><small>Período</small><strong>${brDateOnly(x.period_start)} — ${brDateOnly(x.period_end)}</strong></div><div class="detail-box"><small>Status</small><strong>${financeStatusLabel(x.status)}</strong></div><div class="detail-box"><small>Base</small><strong>${moneyCents(x.base_cents)}</strong></div><div class="detail-box"><small>Ajustes</small><strong>${moneyCents(x.adjustment_cents)}</strong></div><div class="detail-box"><small>Total</small><strong>${moneyCents(x.total_cents)}</strong></div><div class="detail-box"><small>A receber</small><strong>${moneyCents(x.remaining_cents)}</strong></div></div>${x.note?`<div class="detail-note"><small>OBSERVAÇÃO</small><p>${esc(x.note)}</p></div>`:''}${canManage?`<div class="card-actions"><button id="adjustSettlement" class="soft-btn" type="button">+ Ajuste</button>${Number(x.remaining_cents)>0?'<button id="paySettlement" class="gold-btn" type="button">Registrar pagamento</button>':''}</div>`:''}<h3 class="section-mini-title">Lançamentos do período</h3>${(d.entries||[]).map(e=>`<article class="service-stat"><div><h3>${esc(e.appointment?.service||e.description||'Comissão')}</h3><p>${e.appointment?`${esc(e.appointment.customer)} • `:''}${dateTimeBR(e.created_at)}${e.rate_percent!=null?` • ${Number(e.rate_percent).toLocaleString('pt-BR')}%`:''}</p></div><strong>${moneyCents(e.amount_cents)}</strong></article>`).join('')||'<div class="empty">Sem lançamentos.</div>'}<h3 class="section-mini-title">Ajustes</h3>${(d.adjustments||[]).map(a=>`<article class="service-stat"><div><h3>${esc(a.reason)}</h3><p>${dateTimeBR(a.created_at)}${a.actor?` • ${esc(a.actor)}`:''}</p></div><strong>${a.amount_cents>0?'+ ':''}${moneyCents(a.amount_cents)}</strong></article>`).join('')||'<div class="empty">Nenhum ajuste.</div>'}<h3 class="section-mini-title">Pagamentos</h3>${(d.payments||[]).map(p=>`<article class="service-stat"><div><h3>${paymentMethodLabel(p.payment_method)}</h3><p>${dateTimeBR(p.paid_at)}${p.note?` • ${esc(p.note)}`:''}</p></div><strong>${moneyCents(p.amount_cents)}</strong></article>`).join('')||'<div class="empty">Nenhum pagamento registrado.</div>'}`;$('#adjustSettlement')?.addEventListener('click',()=>openCommissionAdjustmentModal(x));$('#paySettlement')?.addEventListener('click',()=>openCommissionPaymentModal(x))}catch(e){$('#modalBody').innerHTML=`<div class="empty">${esc(friendlyError(e))}</div>`}}
function openCommissionAdjustmentModal(x){openModal('AJUSTE DE COMISSÃO',x.professional,`<form id="commissionAdjustmentForm" class="form-grid"><p class="campaign-help">Use valor positivo para bônus e negativo para desconto. Todo ajuste fica registrado no histórico.</p><div class="field"><label>Valor do ajuste</label><input id="commissionAdjustmentValue" inputmode="decimal" placeholder="Ex.: 20,00 ou -15,00" required></div><div class="field"><label>Motivo</label><textarea id="commissionAdjustmentReason" required></textarea></div><button class="gold-btn" type="submit">Registrar ajuste</button></form>`);$('#commissionAdjustmentForm').onsubmit=async e=>{e.preventDefault();try{await rpc('barberium_staff_adjust_commission_settlement',{p_settlement_id:x.id,p_amount_cents:moneyToCents($('#commissionAdjustmentValue').value),p_reason:$('#commissionAdjustmentReason').value.trim()});toast('Ajuste registrado.');await openCommissionSettlementDetail(x.id)}catch(err){toast(friendlyError(err))}}}
function openCommissionPaymentModal(x){const today=isoDate(new Date());openModal('PAGAMENTO DE COMISSÃO',x.professional,`<form id="commissionPaymentForm" class="form-grid"><div class="detail-note"><small>SALDO A RECEBER</small><p><strong>${moneyCents(x.remaining_cents)}</strong></p></div><div class="field"><label>Valor pago</label><input id="commissionPaymentValue" inputmode="decimal" value="${centsInput(x.remaining_cents)}" required></div><div class="field"><label>Forma de pagamento</label><select id="commissionPaymentMethod"><option value="pix">Pix</option><option value="cash">Dinheiro</option><option value="transfer">Transferência</option><option value="other">Outro</option></select></div><div class="field"><label>Data do pagamento</label><input id="commissionPaymentDate" type="date" value="${today}"></div><div class="field"><label>Observação</label><textarea id="commissionPaymentNote"></textarea></div><small class="field-help">Pagamento em dinheiro também sai do caixa aberto da unidade do profissional.</small><button class="gold-btn" type="submit">Registrar pagamento</button></form>`);$('#commissionPaymentForm').onsubmit=async e=>{e.preventDefault();const date=$('#commissionPaymentDate').value;try{await rpc('barberium_staff_record_commission_settlement_payment',{p_settlement_id:x.id,p_amount_cents:moneyToCents($('#commissionPaymentValue').value),p_payment_method:$('#commissionPaymentMethod').value,p_paid_at:date?new Date(`${date}T12:00:00`).toISOString():new Date().toISOString(),p_note:$('#commissionPaymentNote').value.trim()||null});toast('Pagamento de comissão registrado.');await openCommissionSettlementDetail(x.id);if(commissionTab==='settlements')loadCommissionSettlements()}catch(err){toast(friendlyError(err))}}}
async function loadMyCommissions(){
  $('#commissionManagerSubtabs')?.classList.add('hidden');$('#commissionBarberSubtabs')?.classList.remove('hidden');$('#commissionContent').innerHTML='<div class="loading">Carregando suas comissões…</div>';
  try{const d=await rpc('barberium_staff_my_commissions',{});const openEntries=(d.entries||[]).filter(e=>e.status==='open'&&!e.settlement_id),receivable=(d.settlements||[]).filter(x=>x.status!=='paid'),paid=(d.settlements||[]).filter(x=>x.status==='paid');const body=myCommissionFilter==='paid'?`<h3 class="section-mini-title">Acertos pagos</h3>${paid.length?paid.map(x=>`<button class="plan-card commission-settlement-card" data-settlement-id="${x.id}" type="button"><div class="plan-head"><div><span class="plan-tag">Pago</span><h3>${brDateOnly(x.period_start)} — ${brDateOnly(x.period_end)}</h3><p>Quitado ${x.closed_at?dateTimeBR(x.closed_at):''}</p></div><strong>${moneyCents(x.total_cents)}</strong></div></button>`).join(''):'<div class="empty">Nenhum acerto pago neste período.</div>'}`:`<h3 class="section-mini-title">Aguardando fechamento</h3>${openEntries.length?openEntries.map(e=>`<article class="service-stat"><div><h3>${esc(e.description||'Comissão')}</h3><p>${dateTimeBR(e.created_at)}</p></div><strong>${moneyCents(e.amount_cents)}</strong></article>`).join(''):'<div class="empty">Nenhum lançamento aguardando fechamento.</div>'}<h3 class="section-mini-title">Acertos a receber</h3>${receivable.length?receivable.map(x=>`<button class="plan-card commission-settlement-card" data-settlement-id="${x.id}" type="button"><div class="plan-head"><div><span class="plan-tag">${financeStatusLabel(x.status)}</span><h3>${brDateOnly(x.period_start)} — ${brDateOnly(x.period_end)}</h3><p>Pago ${moneyCents(x.paid_cents)} de ${moneyCents(x.total_cents)}</p></div><strong>${moneyCents(x.remaining_cents)}</strong></div></button>`).join(''):'<div class="empty">Nenhum acerto pendente.</div>'}`;$('#commissionContent').innerHTML=`<div class="finance-summary"><article class="finance-kpi gold"><small>A receber</small><strong>${moneyCents(d.open_cents)}</strong></article><article class="finance-kpi"><small>Ainda não fechado</small><strong>${moneyCents(d.unsettled_cents)}</strong></article><article class="finance-kpi"><small>Em acertos</small><strong>${moneyCents(d.in_settlement_cents)}</strong></article></div>${body}`;$$('[data-settlement-id]').forEach(b=>b.onclick=()=>openCommissionSettlementDetail(b.dataset.settlementId))}catch(e){$('#commissionContent').innerHTML=`<div class="empty">${esc(friendlyError(e))}</div>`}
}
...
$$('[data-my-commission-filter]').forEach(b=>b.onclick=()=>{myCommissionFilter=b.dataset.myCommissionFilter;$$('[data-my-commission-filter]').forEach(x=>x.classList.toggle('active',x===b));loadMyCommissions()});
